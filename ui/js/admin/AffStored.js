Schedro.AffStored = function(config) {
	config = config || {};
	
	Ext.apply(this, config)
	Schedro.AffStored.superclass.constructor.call(this);
	this.buildContent();
};

Ext.extend(Schedro.AffStored, Schedro.Functions, {
	grid: null,
	form: null,
	dlg: null,
	pageSize: 20,
	...
	buildContent: function(){
		...
		var ds = new Ext.data.Store({
			proxy: new Ext.data.HttpProxy({url: site_base_url + 'admin/productsAffStored'}),
		...
		var csm = new Ext.grid.CheckboxSelectionModel({
			listeners: {
		            ...
		            fn: function(sm) {
						if (sm.hasSelection()) {
		                    ...
		                    if (selected.get('status') == "shipped") {
		                    	Ext.getCmp('affStoredBtnEdit').show();
		                    	Ext.getCmp('affStoredBtnConfirm').show();
		                    }
		                }
		            }
		            ,scope: this
		        ...
		        'rowdeselect' : {
		        	fn: function(sm) {
		        		Ext.getCmp('affStoredBtnEdit').hide();
		        		Ext.getCmp('affStoredBtnConfirm').hide();
		            }
		        }
		    }
		});
		...
		this.grid = new Ext.grid.GridPanel({
	        ...
	        plugins: [filters],
	        buttons:[{
		        	text: 'Редактировать', id: 'affStoredBtnEdit', selection: '1',
	        		...
	        		text: 'Принять', id: 'affStoredBtnConfirm', selection: 'N',
			...
			title: 'Филиалы &raquo; Приход', buttonsAlign: 'right', frame:true,
			listeners: {
				'render': function(){
