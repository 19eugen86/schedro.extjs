Schedro.Cities = function(config) {
	config = config || {};
	
	Ext.apply(this, config)
	Schedro.Cities.superclass.constructor.call(this);
	this.buildContent();
};

Ext.extend(Schedro.Cities, Schedro.Functions, {
	grid: null,
	form: null,
	dlg: null,
	pageSize: 20,
	...
	buildContent: function(){
		var r = Ext.data.Record.create([
			{name: 'id', type: 'int'}, {name: 'city_name'}, {name: 'country_title'}
		]);
		...
		var cm = new Ext.grid.ColumnModel([
			...
			{header: 'Страна', dataIndex: 'country_title', sortable: true}
		]);
		...
		this.grid = new Ext.grid.GridPanel({
			...
			title: 'Города', buttonsAlign: 'right', /*height: 544,*/ frame:true
		});
	...
	deleteRequest: function(){
		var show_fn = function(btn){
			if('yes' == btn){
				...
				if(sm.hasSelection()){
					...
					Ext.Ajax.request({
	            		url: site_base_url + 'admin/cities',
		...
		Ext.Msg.show({
			title: 'Удалить', msg: 'Вы уверены, что хотите удалить выбранные города?', fn: show_fn,
	...
	showDialog: function(mode){
		...
		var saveData = function(){
			...
			if(f.isValid()){
				...
				Ext.Ajax.request({url: site_base_url + 'admin/cities', params: params, success: this.updateGrid, scope: this});
			}
		}
		...
		var country_cmb = new Ext.form.ComboBox({
            fieldLabel: '<span class="required">*</span> Страна', loadingText: 'Пожалуйста подождите... ', emptyText: 'Пожалуйста, выберите страну...',
            store: new Ext.data.Store({
	            proxy: new Ext.data.HttpProxy({url: site_base_url + 'admin/countries'}),
	            reader: new Ext.data.JsonReader(
            ...
            listeners: {
            	'select': function(cmp){
            		this.form.getForm().findField('f[country_id]').setValue(cmp.getValue());
            	}, scope: this
            },
            valueField:'id', displayField: 'title', pageSize: this.smallPageSize,
            anchor: '100%', minChars: 0, hiddenName: 'f[country_title]', allowBlank: false
		});
		...
		this.form = new Ext.FormPanel({
			...
			items: [
				...
				{xtype: 'hidden', name: 'f[country_id]'},
				{xtype: 'textfield', fieldLabel: '<span class="required">*</span> Название', name: 'f[city_name]', anchor: '100%', allowBlank: false},
				country_cmb
