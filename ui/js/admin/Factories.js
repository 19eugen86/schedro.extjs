Schedro.Factories = function(config) {
	config = config || {};
	
	Ext.apply(this, config)
	Schedro.Factories.superclass.constructor.call(this);
	this.buildContent();
};

Ext.extend(Schedro.Factories, Schedro.Functions, {
	grid: null,
	form: null,
	dlg: null,
	pageSize: 20,
	...
	buildContent: function(){
		...
		var cm = new Ext.grid.ColumnModel([
			csm,
			{header: 'Комбинат', dataIndex: 'title', width: 130, sortable: true},
		...
		this.grid = new Ext.grid.GridPanel({
			...
			title: 'Комбинаты', buttonsAlign: 'right', /*height: 544,*/ frame:true
		});
	...
	deleteRequest: function(){
		var show_fn = function(btn){
			if('yes' == btn){
				...
				if(sm.hasSelection()){
					...
					Ext.Ajax.request({
	            		url: site_base_url + 'admin/factories',
		...
		Ext.Msg.show({
			title: 'Удалить', msg: 'Вы уверены, что хотите удалить выбранные комбинаты?', fn: show_fn,
	...
	showDialog: function(mode){
		...
		var saveData = function(){
			...
			if(f.isValid()){
				...
				Ext.Ajax.request({url: site_base_url + 'admin/factories', params: params, success: this.updateGrid, scope: this});
			}
		}
		...
		this.form = new Ext.FormPanel({
			...
			items: [
				...
				{xtype: 'textfield', fieldLabel: '<span class="required">*</span> Название комбината', name: 'f[title]', anchor: '100%', allowBlank: false},
