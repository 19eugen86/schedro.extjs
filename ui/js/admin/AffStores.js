Schedro.AffStores = function(config) {
	config = config || {};
	
	Ext.apply(this, config)
	Schedro.AffStores.superclass.constructor.call(this);
	this.buildContent();
};

Ext.extend(Schedro.AffStores, Schedro.Functions, {
	grid: null,
	form: null,
	dlg: null,
	pageSize: 20,
		...
		var show_fn = function(btn){
			if('yes' == btn){
				...
				if(sm.hasSelection()){
					...
					Ext.Ajax.request({
	            		url: site_base_url + 'admin/stores',
	...
	showDialog: function(mode){
		...
		var saveData = function(){
			...
			if(f.isValid()){
				...
				Ext.Ajax.request({url: site_base_url + 'admin/stores', params: params, success: this.updateGrid, scope: this});
			}
		}
		...
		var affCmb = new Ext.form.ComboBox({
            ...
            listeners: {
            	'select': function(cmp){
					this.form.getForm().findField('f[aff_id]').setValue(cmp.getValue());
            	}, scope: this
            },
