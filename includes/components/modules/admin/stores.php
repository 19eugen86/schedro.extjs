...
$entity = EntityFactory::loadEntity('Stores');

$cmd = (!empty($_REQUEST['cmd'])) ? $_REQUEST['cmd'] : 'getGrid';
if ($cmd == 'getAffStores' && !empty($_REQUEST['aff_id'])) {
	$data = $entity->getGroupedBy('aff_id', intval($_REQUEST['aff_id']));
} else {
