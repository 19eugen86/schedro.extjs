Schedro.ProductGroups = function(config) {
	config = config || {};
	
	Ext.apply(this, config)
	Schedro.ProductGroups.superclass.constructor.call(this);
	this.buildContent();
};

Ext.extend(Schedro.ProductGroups, Schedro.Functions, {
	grid: null,
	form: null,
	dlg: null,
	pageSize: 20,
	...
	buildContent: function(){
		...
		this.grid = new Ext.grid.GridPanel({
			...
			title: 'Товарные группы', buttonsAlign: 'right', /*height: 544,*/ frame:true
		});
	...
	deleteRequest: function(){
		var show_fn = function(btn){
			if('yes' == btn){
				...
				if(sm.hasSelection()){
					...
					Ext.Ajax.request({
	            		url: site_base_url + 'admin/productGroups',
		...
		Ext.Msg.show({
			title: 'Удалить', msg: 'Вы уверены, что хотите удалить выбранные группы?', fn: show_fn,
	...
	showDialog: function(mode){
		...
		var saveData = function(){
			...
			if(f.isValid()){
				...
				Ext.Ajax.request({url: site_base_url + 'admin/productGroups', params: params, success: this.updateGrid, scope: this});
			}
		}
