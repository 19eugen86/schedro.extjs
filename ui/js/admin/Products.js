Schedro.Products = function(config) {
	config = config || {};
	
	Ext.apply(this, config)
	Schedro.Products.superclass.constructor.call(this);
	this.buildContent();
};

Ext.extend(Schedro.Products, Schedro.Functions, {
	grid: null,
	form: null,
	dlg: null,
	pageSize: 20,
	...
	buildContent: function(){
		var r = Ext.data.Record.create([
			{name: 'id', type: 'int'}, {name: 'title'}, {name: 'group_id'}, {name: 'group_title'}
		]);
		...
		var cm = new Ext.grid.ColumnModel([
			csm,
			{header: 'Наименование', dataIndex: 'title', width: 130, sortable: true},
			{header: 'Товарная группа', dataIndex: 'group_title', sortable: true}
		]);
		...
		this.grid = new Ext.grid.GridPanel({
			...
			title: 'Продукция', buttonsAlign: 'right', /*height: 544,*/ frame:true
		});
		...
		var show_fn = function(btn){
			if('yes' == btn){
				...
				if(sm.hasSelection()){
					...
					Ext.Ajax.request({
	            		url: site_base_url + 'admin/products',
	...
	showDialog: function(mode){
		...
		var saveData = function(){
			...
			if(f.isValid()){
				...
				Ext.Ajax.request({url: site_base_url + 'admin/products', params: params, success: this.updateGrid, scope: this});
			}
		}
		...
		if('edit' == mode){
			var userGroupFieldMark = 'Товарная группа';
		} else {
			var userGroupFieldMark = '<span class="required">*</span> Товарная группа';
		}
		...
		var product_group_cmb = new Ext.form.ComboBox({
            ...
            anchor: '100%', minChars: 0, hiddenName: 'f[group_title]'
		});
		...
		this.form = new Ext.FormPanel({
			...
			items: [
				...
				{xtype: 'textfield', fieldLabel: '<span class="required">*</span> Наименование', name: 'f[title]', anchor: '100%', allowBlank: false},
				product_group_cmb
			]
		});
