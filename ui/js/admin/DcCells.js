Schedro.DcCells = function(config) {
	config = config || {};
	
	Ext.apply(this, config)
	Schedro.DcCells.superclass.constructor.call(this);
	this.buildContent();
};

Ext.extend(Schedro.DcCells, Schedro.Functions, {
	grid: null,
	form: null,
	dlg: null,
	pageSize: 20,
		...
		var show_fn = function(btn){
			if('yes' == btn){
				...
				if(sm.hasSelection()){
					...
					Ext.Ajax.request({
	            		url: site_base_url + 'admin/dcCells',
	...
	showDialog: function(mode){
		...
		var saveData = function(){
			...
			if(f.isValid()){
				...
				Ext.Ajax.request({url: site_base_url + 'admin/dcCells', params: params, success: this.updateGrid, scope: this});
			}
		}
		...
		var store_cmb = new Ext.form.ComboBox({
            ...
            listeners: {
            	'select': function(cmp){
            		this.form.getForm().findField('f[dc_store_id]').setValue(cmp.getValue());
            	}, scope: this
            },
            valueField:'id', displayField: 'title', pageSize: this.smallPageSize,
            anchor: '100%', minChars: 0, name: 'f[dc_store_title]', allowBlank: false
		});
		
		var dcCmb = new Ext.form.ComboBox({
            ...
            listeners: {
            	'select': function(cmp){
					store_cmb.getStore().setBaseParam('dc_id', cmp.getValue());
            	}, scope: this
            },
            valueField:'id', displayField: 'title', pageSize: this.smallPageSize,
		...
		this.form = new Ext.FormPanel({
			...
			items: [
				...
				{xtype: 'hidden', name: 'f[dc_store_id]'},
