...
class CarrierVehicleEntity extends AbstractEntity
{
	protected $dbTable = 'carriers_vehicles';
	...
	public function __construct($id = null) {
		...
		$this->dbUniqueValues = array(
			'reg_number' => 'Автомобиль с таким регистрационным номером уже существует'
		);
	...
	public function getGroupedBy($key, $value, $filterTable = '') {
		...
		if (!empty($all) && is_array($all)) {
			foreach ($all as $key => $value) {
				$all[$key]['title'] = $value['make'].' '.$value['type'].' ['.$value['reg_number'].']';
			}
		}
		return array(
			'data' => $all,
	...
	public function getOneItem($id = null) {
		...
		if (!empty($result) && is_array($result)) {
			foreach ($result as $key => $value) {
				$result[$key]['title'] = $value['make'].' '.$value['type'].' ['.$value['reg_number'].']';
			}
		}
