Schedro.UserGroups = function(config) {
	config = config || {};
	
	Ext.apply(this, config)
	Schedro.UserGroups.superclass.constructor.call(this);
	this.buildContent();
};

Ext.extend(Schedro.UserGroups, Schedro.Functions, {
	grid: null,
	form: null,
	dlg: null,
	pageSize: 20,
	...
	buildContent: function(){
		...
		this.grid = new Ext.grid.GridPanel({
			...
			title: 'Список групп', buttonsAlign: 'right', /*height: 544,*/ frame:true
		});
		...
		var show_fn = function(btn){
			if('yes' == btn){
				...
				if(sm.hasSelection()){
					...
					Ext.Ajax.request({
	            		url: site_base_url + 'admin/userGroups',
	...
	showDialog: function(mode){
		...
		var saveData = function(){
			...
			if(f.isValid()){
				...
				Ext.Ajax.request({url: site_base_url + 'admin/userGroups', params: params, success: this.updateGrid, scope: this});
			}
		}
