Schedro.DcShipped = function(config) {
	config = config || {};
	
	Ext.apply(this, config)
	Schedro.DcShipped.superclass.constructor.call(this);
	this.buildContent();
};

Ext.extend(Schedro.DcShipped, Schedro.Functions, {
	grid: null,
	form: null,
	dlg: null,
	pageSize: 20,
	...
	buildContent: function(){
		...
		var ds = new Ext.data.Store({
			proxy: new Ext.data.HttpProxy({url: site_base_url + 'admin/productsDcShipped'}),
		...
		var csm = new Ext.grid.CheckboxSelectionModel({
			listeners: {
		            ...
		            fn: function(sm) {
						if (sm.hasSelection()) {
		                    ...
		                    if (selected.get('status') == "in_progress") {
		                    	Ext.getCmp('dcShippedBtnEdit').show();
		                    	Ext.getCmp('dcShippedBtnConfirm').show();
		                    	Ext.getCmp('dcShippedBtnDelete').show();
		                    }
		                }
		            }
		            ,scope: this
		        ...
		        'rowdeselect' : {
		        	fn: function(sm) {
		        		Ext.getCmp('dcShippedBtnEdit').hide();
		        		Ext.getCmp('dcShippedBtnConfirm').hide();
		        		Ext.getCmp('dcShippedBtnDelete').hide();
		            }
		        }
		    }
		});
		...
		this.grid = new Ext.grid.GridPanel({
	        ...
	        plugins: [filters],
	        buttons:[{
	        		text: 'Редактировать', id: 'dcShippedBtnEdit', selection: '1',
	        		...
	        		text: 'Отправить', id: 'dcShippedBtnConfirm', selection: '1',
	        		...
	        		text: 'Удалить', id: 'dcShippedBtnDelete', selection: '1',
			...
			title: 'РЦ &raquo; Расход', buttonsAlign: 'right', frame:true,
			listeners: {
				'render': function(){
...
//		this.reloadGrid();
	...
	showDialog: function(mode){
		...
		var carriersDriversOutCmb = new Ext.form.ComboBox({
            ...
            listeners: {
            	'select': function(cmp){
					shipForm.getForm().findField('f[carrier_driver_id]').setValue(cmp.getValue());
            	}, scope: this
            },
            valueField:'id', displayField: 'fullname', pageSize: this.smallPageSize,
            anchor: '100%', minChars: 0, name: 'f[carrier_driver_fullname]', allowBlank: false,
            listWidth: 300, width: 50
		});
		...
		var carriersVehiclesOutCmb = new Ext.form.ComboBox({
            ...
            listeners: {
            	'select': function(cmp){
					shipForm.getForm().findField('f[carrier_vehicle_id]').setValue(cmp.getValue());
            	}, scope: this
            },
            valueField:'id', displayField: 'title', pageSize: this.smallPageSize,
            anchor: '100%', minChars: 0, name: 'f[carrier_vehicle_title]', allowBlank: false,
            listWidth: 300, width: 50
		});
		...
		var carriersOutCmb = new Ext.form.ComboBox({
            ...
            listeners: {
            	'select': function(cmp){
					shipForm.getForm().findField('f[carrier_id]').setValue(cmp.getValue());
					
					carriersDriversOutCmb.clearValue();
					carriersDriversOutCmb.getStore().setBaseParam('carrier_id', cmp.getValue());
					
					carriersVehiclesOutCmb.clearValue();
					carriersVehiclesOutCmb.getStore().setBaseParam('carrier_id', cmp.getValue());
            	}, scope: this
            },
            valueField:'id', displayField: 'title', pageSize: this.smallPageSize,
            anchor: '100%', minChars: 0, name: 'f[carrier_title]', allowBlank: false,
            listWidth: 300, width: 50
		});
		//
		...
		var shipForm = new Ext.FormPanel({
			...
			items: [dcOutCmb, {
				...
				items:[
				       ...
				       {xtype: 'hidden', name: 'f[carrier_driver_id]'},
				       {xtype: 'hidden', name: 'f[carrier_vehicle_id]'},
				       ...
				       {xtype: 'container', columnWidth:.1, style: 'margin: 0px 2px 0px 0px;', layout: 'anchor', items: [{xtype: 'textfield', emptyText: 'Кол-во', width: 50, name: 'f[quantity]', allowBlank: false, anchor:'100%'}]},
		...
		if(sm.hasSelection()){
			Ext.Ajax.request({
	               ...
	               success: function(response){
	               	...
	               	productOutCmb.getStore().setBaseParam('dc_id', data.from_id);
					productGroupOutCmb.getStore().setBaseParam('dc_id', data.from_id);
					productPartsOutCmb.getStore().setBaseParam('dc_id', data.from_id);
	               	recipientsOutCmb.getStore().setBaseParam('direction_selected', data.direction);
	               	carriersDriversOutCmb.getStore().setBaseParam('carrier_id', data.carrier_id);
					carriersVehiclesOutCmb.getStore().setBaseParam('carrier_id', data.carrier_id);
