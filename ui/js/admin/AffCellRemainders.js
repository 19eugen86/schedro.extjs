Schedro.AffCellRemainders = function(config) {
	config = config || {};
	
	Ext.apply(this, config)
	Schedro.AffCellRemainders.superclass.constructor.call(this);
	this.buildContent();
};

Ext.extend(Schedro.AffCellRemainders, Schedro.Functions, {
	grid: null,
	form: null,
	dlg: null,
	pageSize: 20,
	...
	buildContent: function(){
		var r = Ext.data.Record.create([
			...
			{name: 'aff_title'}, {name: 'aff_store_title'}, {name: 'aff_cell_title'},
			{name: 'group_title'}, {name: 'date_formated'}
		]);
	
		var ds = new Ext.data.Store({
			proxy: new Ext.data.HttpProxy({url: site_base_url + 'admin/productsCellRemainders'}),
		...
		var filters = new Ext.ux.grid.GridFilters({
	  		filters:[
	    		...
	    		{type: 'string', dataIndex: 'aff_title'},
	    		{type: 'string', dataIndex: 'aff_store_title'},
	    		{type: 'string', dataIndex: 'aff_cell_title'},
		...
		var cm = new Ext.grid.ColumnModel([
			...
			{header: 'Камера', dataIndex: 'aff_cell_title', sortable: true},
	...
	showDialog: function(mode){
		...
		var saveData = function(){
			var ship_fn = function() {
				...
				if(f.isValid()){
					...
					Ext.Ajax.request({url: site_base_url + 'admin/productsCellShipped', params: params, success: this.updateGrid, scope: this});
				}
			}
			Ext.Msg.show({
		...
		var cellsCmb = new Ext.form.ComboBox({
            ...
            store: new Ext.data.Store({
	        	...
	        	baseParams: {cmd: 'getAffStoreCells'},
                remoteSort: true
            }),
            listeners: {
            	'select': function(cmp){
					...
					shipForm.getForm().findField(fieldName).setValue(cmp.getValue());
					productGroupOutCmb.getStore().setBaseParam('cell_id', cmp.getValue());
					productOutCmb.getStore().setBaseParam('cell_id', cmp.getValue());
					productPartsOutCmb.getStore().setBaseParam('cell_id', cmp.getValue());
            	}, scope: this
            },
            valueField:'id', displayField: 'title', pageSize: this.smallPageSize,
				...
				var cellsCmbCloned = cellsCmb.cloneConfig({
					listeners: {
		            	'select': function(cmp){
							var fieldName = cmp.getName().replace('aff_cell_title', 'aff_cell_id');
							shipForm.getForm().findField(fieldName).setValue(cmp.getValue());
							productGroupOutCmbCloned.getStore().setBaseParam('cell_id', cmp.getValue());
							productOutCmbCloned.getStore().setBaseParam('cell_id', cmp.getValue());
							productPartsOutCmbCloned.getStore().setBaseParam('cell_id', cmp.getValue());
		            	}, scope: this
		            },
