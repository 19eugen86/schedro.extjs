Schedro.CarriersVehicles = function(config) {
	config = config || {};
	
	Ext.apply(this, config)
	Schedro.CarriersVehicles.superclass.constructor.call(this);
	this.buildContent();
};

Ext.extend(Schedro.CarriersVehicles, Schedro.Functions, {
	grid: null,
	form: null,
	dlg: null,
	pageSize: 20,
	...
	buildContent: function(){
		var r = Ext.data.Record.create([
			{name: 'id', type: 'int'}, {name: 'make'}, {name: 'type'}, {name: 'reg_number'}, {name: 'carrier_id'}, {name: 'carrier_title'}
		]);
		...
		var filters = new Ext.ux.grid.GridFilters({
	  		filters:[
	    		...
	    		{type: 'string', dataIndex: 'reg_number'},
		...
		var cm = new Ext.grid.ColumnModel([
			csm,
			{header: 'Марка', dataIndex: 'make', width: 130, sortable: true},
			{header: 'Модель', dataIndex: 'type', sortable: true},
			{header: 'Рег. номер', dataIndex: 'reg_number', width: 130, sortable: true},
	...
	deleteRequest: function(){
		var show_fn = function(btn){
			if('yes' == btn){
				...
				if(sm.hasSelection()){
					...
					Ext.Ajax.request({
	            		url: site_base_url + 'admin/carriersVehicles',
		...
		Ext.Msg.show({
			title: 'Удалить', msg: 'Вы уверены, что хотите удалить выбранные автомобили?', fn: show_fn,
	...
	showDialog: function(mode){
		...
		var saveData = function(){
			...
			if(f.isValid()){
				...
				Ext.Ajax.request({url: site_base_url + 'admin/carriersVehicles', params: params, success: this.updateGrid, scope: this});
			}
		}
		...
		this.form = new Ext.FormPanel({
			...
			items: [
				...
				{xtype: 'textfield', fieldLabel: '<span class="required">*</span> Марка', name: 'f[make]', anchor: '100%', allowBlank: false},
				{xtype: 'textfield', fieldLabel: '<span class="required">*</span> Модель', name: 'f[type]', anchor: '100%', allowBlank: false},
				{xtype: 'textfield', fieldLabel: '<span class="required">*</span> Рег. номер', name: 'f[reg_number]', anchor: '100%', allowBlank: false},
				carrier_cmb
			]
		});
