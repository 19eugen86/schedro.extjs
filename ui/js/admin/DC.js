Schedro.DC = function(config) {
	config = config || {};
	
	Ext.apply(this, config)
	Schedro.DC.superclass.constructor.call(this);
	this.buildContent();
};

Ext.extend(Schedro.DC, Schedro.Functions, {
	grid: null,
	form: null,
	dlg: null,
	pageSize: 20,
	...
	buildContent: function(){
		...
		var cm = new Ext.grid.ColumnModel([
			csm,
			{header: 'РЦ', dataIndex: 'title', width: 130, sortable: true},
		...
		this.grid = new Ext.grid.GridPanel({
			...
			title: 'Распределительные Центры', buttonsAlign: 'right', /*height: 544,*/ frame:true
		});
	...
	deleteRequest: function(){
		...
		Ext.Msg.show({
			title: 'Удалить', msg: 'Вы уверены, что хотите удалить выбранные РЦ?', fn: show_fn,
	...
	showDialog: function(mode){
		...
		var saveData = function(){
			...
			if(f.isValid()){
				...
				Ext.Ajax.request({url: site_base_url + 'admin/dc', params: params, success: this.updateGrid, scope: this});
			}
		}
		...
		this.form = new Ext.FormPanel({
			...
			items: [
				...
				{xtype: 'textfield', fieldLabel: '<span class="required">*</span> РЦ', name: 'f[title]', anchor: '100%', allowBlank: false},
